 ...
 * Project 2: Yahtzee 6.0 (continued from project 1)
 * Additions since Project 1 V5: vectors, arrays, and functions used to store, sort, and search
 * for scores and names from files
 * Created on February 3, 2018, 4:00 PM
 */
//System Libraries
...
int main(int argc, char** argv) {
    ...
    int upper, total, bonus, score, rolls, formerScore, past, arsize, position; //scores, bonus score, and number of rolls used in round
    char r; //user-inputted character to roll the dice
    string choice, player;//user inputted scoring category
    ...
    bubbleSort(scores, arsize, players);
    cout<<"Score Board"<<endl;
    for(int i = 0; i<arsize; i++)
        cout<<setw(8)<<players[i]<<setw(10)<<scores[i]<<endl;
    cout<<"What is your first name?\n";
    cin>>player;
    //search for the player's name in the score board
    position = linearSearch(players, arsize, player);
    if(position!=-1){
        cout<<"You've played before. This was your last score: "<<scores[position]<<endl;
    }
    else
        cout<<"You haven't played before (at least with that name). Good luck on your first run!\n";
    //do 13 rounds 
